...
class BNN:
    ...
    def _compile_accuracy(self, inputs, targets):
        ...
        """
        prob, _ = self._compile_outputs(inputs)
        pred_y = tf.math.round(prob)

        accuracy_bool = tf.math.equal(targets, pred_y)
        accuracy = tf.cast(accuracy_bool, tf.float32)
        accuracy = tf.reduce_mean(accuracy, axis=(1,2))
...
#                 self.pred_accuracy = self._compile_accuracy(self.sy_train_in, self.sy_train_targ)
#             if self.evaluate_classifier:
#                 self.mse = self._mse_loss(self.sy_train_in, self.sy_train_targ, inc_var_loss=False)
#                 self.pred_prob = self._compile_prob(self.sy_train_in)
...
#             model_metrics = {'val_acc': holdout_acc.mean(), 'val_loss': val_loss}
#         elif self.evaluate_classifier:
#             model_metrics = {'val_prob': holdout_prob.mean(), 'val_mse': holdout_mse.mean(), 'val_loss': val_loss}
#         else:
#             model_metrics = {'val_loss': val_loss}
...
#         # nce = tf.math.log((1. - prob) + self.eps)
#         # nce = tf.reduce_mean(nce, axis=(1,2))
#         # return nce
...
#         """
#         prob, _ = self._compile_outputs(inputs)
#         ce = - targets * tf.math.log(prob + self.eps)  - (1. - targets) * tf.math.log((1. - prob) + self.eps)
...
#     def _compile_accuracy(self, inputs, targets, real=True):
#         """Helper method for compiling the holdout accuracy.
...
#         Returns: (tf.Tensor) A tensor representing the loss on the input arguments.
#         """
#         # keep = tf.equal(targets, real)
#         # indices = tf.where(keep)
#         # inputs = tf.gather(inputs, keep, axis=1)

#         # prob, _ = self._compile_outputs(inputs)
#         # return tf.reduce_mean(prob, axis=(1,2))
